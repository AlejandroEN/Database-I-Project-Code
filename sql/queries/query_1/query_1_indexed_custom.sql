...
--- CREATE INDEX idx_matricula_year ON matricula USING HASH (year);
--- DROP INDEX IF EXISTS idx_matricula_year;
--- CREATE INDEX idx_matricula_alumno_dni ON matricula(alumno_dni);
--- DROP INDEX IF EXISTS idx_matricula_alumno_dni;
--- CREATE INDEX idx_sede_construccion_fecha ON sede(construccion_fecha);
...
WHERE
    sede.id IN (
        ...
        WHERE sedes_filtradas.numero_alumnos <= 300 AND sedes_filtradas.numero_alumnos >= 200
    )
ORDER BY
    sede.id;
DROP INDEX IF EXISTS idx_sede_construccion_fecha;
